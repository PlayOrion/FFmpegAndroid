...
void player_print_codec_description(const struct AVCodec *codec)
{
	...
	LOGI(10, "player_print_codec_description id: %d codec: %s, type: %s",
...
static void player_print_subtitle(AVSubtitle *sub, int64_t time)
{
	...
	LOGI(3, "player_decode_subtitles sub.start_display_time: %d",
			sub->start_display_time);
	LOGI(3, "player_decode_subtitles sub.end_display_time: %d",
	...
	for (i = 0; i < sub->num_rects; ++i)
	{
		...
		LOGI(3, "player_decode_subtitles --rect->(x,y,w,h) = (%d,%d,%d,%d)",
				rect->x, rect->y, rect->w, rect->h);
		LOGI(3, "player_decode_subtitles --rect->nb_colors = %d",
				rect->nb_colors);
		LOGI(3, "player_decode_subtitles --rect->text = %s", rect->text);
		LOGI(3, "player_decode_subtitles --rect->ass = %s", rect->ass);
		LOGI(3, "player_decode_subtitles --rect->forced = %s",
...
void player_update_current_time(struct State *state, int64_t current_time,
		int64_t video_duration, int is_finished)
{
	struct Player *player = state->player;
	jboolean jis_finished = is_finished ? JNI_TRUE : JNI_FALSE;

	LOGI(4, "player_update_current_time: %f/%f", current_time / 1000000.0,
			video_duration / 1000000.0);
	(*state->env)->CallVoidMethod(state->env, player->thiz,
			player->player_on_update_time_method, current_time, video_duration,
			jis_finished);
}
...
void player_update_time(struct State *state, int is_finished)
{
	...
	LOGI(6, "player_update_time: %f/%f", current_video_time / 1000000.0,
			video_duration / 1000000.0);
	...
	if (inform_user)
	{
		...
		player_update_current_time(state, current_video_time, video_duration,
				is_finished);
	}
}

enum WaitFuncRet player_wait_for_frame(struct Player *player,
		int64_t stream_time, int stream_no)
{
	...
	while (1)
	{
		...
		LOGI(8, "player_wait_for_frame[%d = %s] = (%f) - (%f)", stream_no,
				player->video_stream_no == stream_no ? "Video" : "Audio",
				stream_time / 1000000.0, current_video_time / 1000000.0);
		...
		struct PacketData *packet_data)
{
	...
	LOGI(6, "Buffer: width: %d, height: %d, stride: %d", buffer.width,
...
void * player_read_from_stream(void *data)
{
	...
	int interrupt_ret;
	JavaVMAttachArgs thread_spec =
	{ JNI_VERSION_1_4, "FFmpegReadFromStream",
	NULL };
	...
	for (;;)
	{
		...
		*packet_data->packet = packet;
		...
		seek_target = av_rescale_q(player->seek_position, AV_TIME_BASE_Q
		, seek_input_stream->time_base);
		LOGI(3, "player_read_from_stream seeking to: "
				"%ds, time_base: %f", player->seek_position / 1000000.0,
				seek_target);
...
//				av_q2d(stream->time_base) * pts;
	...
	if (ret < 0)
	{
		err = -ERROR_PLAYING_AUDIO;
		LOGE(3,
				"Could not write audio track: reason: %d look in AudioTrack.write()",
				ret);
		goto free_local_ref;
	}
		...
		const struct AVCodec **codec, int stream_no)
{
	...
	*codec = avcodec_find_decoder(codec_id);
	if (*codec == NULL)
	{
		LOGE(1, "player_set_data_source Could not find codec for id: %d",
				codec_id);
	...
	LOGI(3, "player_open_stream opened: %d, name: %s, long_name: %s", codec_id,
		...
		int recommended_stream_no)
{
	...
	{
		LOGI(5, "player_set_data_source Video size is [%d x %d]",
		...
		const char *file_path)
{
	if (LOG_LEVEL >= 3)
	{
		...
		LOGI(3, "player_set_data_source Number of streams: %d",
...
int player_create_audio_track(struct Player *player, struct State *state)
{
	...
	jobject audio_track = (*state->env)->CallObjectMethod(state->env,
			player->thiz, player->player_prepare_audio_track_method,
			...
			|| ctx->sample_rate != audio_track_sample_rate)
	{
		...
		LOGI(3, "player_set_data_sourcd preparing conversion of %d Hz %s %d "
		...
		if (!player->swr_context || swr_init(player->swr_context) < 0)
		{
			LOGE(1, "Cannot create sample rate converter for conversion "
					...
					ctx->channels, audio_track_sample_rate,
					av_get_sample_fmt_name(player->audio_track_format),
					player->audio_track_channel_count);
...
void player_get_video_duration(struct Player *player)
{
	...
	for (i = 0; i < player->caputre_streams_no; ++i)
	{
		...
		if (stream->duration > 0)
		{
			...
			LOGI(3, "player_set_data_source stream[%d] duration: %ld", i,
					stream->duration);
			return;
		}
	}
	if (player->input_format_ctx->duration != 0)
	{
		...
		LOGI(3, "player_set_data_source video duration: %ld",
...
int player_prepare_ass_decoder(struct Player* player, const char *font_path)
{
	...
	LOGI(3, "player_prepare_ass_decoder: setting ass default font to: %s",
			font_path);
...
int player_create_interrupt_callback(struct Player *player)
{
	...
	player->interrupt_callback = (AVIOInterruptCB
			)
			{ player_ctx_interrupt_callback, player };
		...
		int subtitle_stream_no)
{
	...
	{ stream_no : player->video_stream_no, player: player, env:state->env };
#ifdef SUBTITLES
	if (player->subtitle_stream_no >= 0)
	{
		struct DecoderState subtitle_decoder_state =
				{ stream_no : player->subtitle_stream_no, player: player, env
						:state->env };
...
#warning "Profiler enabled"
	...
	free_player: if (player->thiz != NULL)
	{
		(*env)->DeleteGlobalRef(env, player->thiz);
	}
	if (player->audio_track_class != NULL)
...
void jni_player_render(JNIEnv *env, jobject thiz, jobject surface)
{
	...
	{
		LOGE(1, "jni_player_render Window have to be null before "
